...
if ($method === 'GET') {
    ...
    if (isset($_GET['slug'])) {
        ...
        $stmt->execute([$slug]);
        $link = $stmt->fetch();
        if (!$link) { http_response_code(404); echo json_encode(['ok'=>false]); exit; }
        $cfg = getConfig($pdo);
        ...
        echo json_encode([
            ...
            'addonsPrice'   => (int)($cfg['addons_price'] ?? 300),
        ]);
        exit;
    }
    ...
    // Admin config fetch
    session_start();
    if (empty($_SESSION['admin_authed'])) { http_response_code(403); echo json_encode(['ok'=>false,'error'=>'Forbidden']); exit; }
    $cfg = getConfig($pdo);
    ...
    echo json_encode([
        ...
        'onDelivery'    => (int)$cfg['on_delivery'],
        'whatsappLink'  => $cfg['whatsapp_link']  ?? '',
        'instagramLink' => $cfg['instagram_link'] ?? '',
        'addonsPrice'   => (int)($cfg['addons_price'] ?? 300),
    ]);
...
if ($method === 'POST') {
    ...
    $onDelivery    = (int)($_POST['onDelivery']     ?? $existing['on_delivery']    ?? 4000);
    ...
    $addonsPrice   = (int)($_POST['addonsPrice']    ?? $existing['addons_price']   ?? 300);
    ...
    if (!empty($_FILES['qrImage']['tmp_name']) && $_FILES['qrImage']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed, true)) {
            ...
            if (move_uploaded_file($_FILES['qrImage']['tmp_name'], $dest)) {
                if ($qrImage && file_exists(__DIR__ . '/../uploads/' . basename($qrImage))) @unlink(__DIR__ . '/../uploads/' . basename($qrImage));
                $qrImage = $filename;
            }
        }
    }
    ...
    $stmt = $pdo->prepare('UPDATE `config` SET upi_id=?, qr_image=?, total_price=?, advance_amount=?, on_delivery=?, whatsapp_link=?, instagram_link=?, addons_price=? WHERE id=1');
    $stmt->execute([$upiId, $qrImage, $totalPrice, $advanceAmount, $onDelivery, $whatsappLink, $instagramLink, $addonsPrice]);
