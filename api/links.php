...
// GET — now served by orders.php unified endpoint; keep thin compat layer
if ($method === 'GET') {
    if (!$channel && !empty($_GET['channel'])) {
        $ch = $_GET['channel'];
    ...
    // Return empty array — frontend should call orders.php for the unified list
    echo json_encode([]);
    exit;
}
...
if ($method === 'POST') {
    ...
    for ($i = 0; $i < 20; $i++) {
        ...
        if (!$chk->fetch()) { $slug = $candidate; break; }
    }
    if (!$slug) {
